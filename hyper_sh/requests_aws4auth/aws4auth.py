...
/*
   Modified to import hyper signature changes
...
class AWS4Auth(AuthBase):
    ...
    def __call__(self, req):
        ...
        req.headers['x-hyper-content-sha256'] = content_hash.hexdigest()
        if self.session_token:
    ...
    def get_request_date(cls, req):
        ...
        """
        date = None
        for header in ['x-hyper-date', 'date']:
    ...
    def encode_body(req):
        ...
        if isinstance(req.body, text_type):
            ...
            if len(split) == 2:
                ...
                ct = split[0]
                if (ct == 'application/x-www-form-urlencoded' or 'x-hyper-' in ct):
    ...
    def get_canonical_request(self, req, cano_headers, signed_headers):
        ...
        req            -- Requests PreparedRequest object. Should already
                          include an x-hyper-content-sha256 header
    ...
    def get_canonical_headers(cls, req, include=None):
        ...
        include -- List of headers to include in the canonical and signed
                   ...
                   includes host, content-type and any header starting 'x-hyper-'
                   except for x-hyper-client context, which appears to break
                   mobile analytics auth if included. Except for the
                   x-hyper-client-context exclusion these defaults are per the
                   AWS documentation.
    ...
    def get_sig_string(req, cano_req, scope):
        ...
        """
        amz_date = req.headers['x-hyper-date']
        hsh = hashlib.sha256(cano_req.encode())
        sig_items = ['HYPER-HMAC-SHA256', amz_date, scope, hsh.hexdigest()]
        sig_string = '\n'.join(sig_items)
        return sig_string
